...
- `flowpipe-core` — the library itself. Runtime dependencies: `slf4j-api`, `dev.failsafe:failsafe:3.3.2` (resilience engine — retry, timeout, circuit breaker; kept as a private engine detail, never exposed in the public API).
  - `io.flowpipe.api` — public surface: `Step`, `StepDescriptor`, `StepContext`, `Result`, `Success`, `Failure`, `ExecutionTrace`, `TraceEntry`, `TriFunction`, `QuadFunction`, `RetryPolicy`, `PipelineLifecycle<I,O>`, `TimeoutPolicy`, `StepTimeoutException`, `CircuitBreakerPolicy`, `CircuitBreakerOpenException`, `PipelineDescriptor`, `NodeDescriptor`.
