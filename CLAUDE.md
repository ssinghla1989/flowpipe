...
- Parallel composition: `parallel2(Class<R>, combiner, stepA, stepB)` / `parallel3` / `parallel4` (typed combiners — `Class<R>` is the first argument and enables downstream type-compatibility checking at `build()` time), `parallelN` (variadic), `withExecutor(ExecutorService)`; retry, timeout, and circuit-breaker policies declared on a parallel branch step are fully honored — the branch runs through the same resilience machinery as sequential steps
