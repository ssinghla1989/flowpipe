...
Defines the `PipelineLifecycle<I, O>` hook interface that gives callers visibility into pipeline execution boundaries — before any step runs (`onStart`), after execution completes regardless of outcome (`onFinish`), and additionally when the result is a failure (`onError`). Specifies the invocation contract, exception-isolation rules, and the test-utility recorder.
