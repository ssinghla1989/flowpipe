...
Defines how a developer wires conditional logic into a pipeline via `branch()`, how the engine evaluates the predicate at runtime to route execution to one of two typed arm pipelines, and the trace, log, and metrics semantics for both the taken and non-taken arms.
