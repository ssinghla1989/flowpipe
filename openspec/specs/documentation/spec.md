...
Defines the accuracy requirements for FlowPipe's developer-facing documentation — `README.md` and `CLAUDE.md` — so that every shipped feature is discoverable, illustrated with a minimal code example, and correctly reflected in the project's machine-readable guidance for Claude Code sessions.
