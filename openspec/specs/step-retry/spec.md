...
Defines per-step configurable retry-with-exponential-backoff — the `RetryPolicy` value type, the `StepDescriptor.withRetry(...)` wiring API, and the engine retry loop that re-invokes `step.execute(...)` transparently on failure — so step authors write zero retry-related code and every step benefits from framework-managed resilience.
